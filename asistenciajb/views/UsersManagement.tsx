...
import { createPortal } from "react-dom"; // 🚀 NUEVO: Importamos createPortal
...
const UsersManagement: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-jbGray font-medium mt-1">
            Directorio de personal y permisos del sistema JB.
          </p>
        </div>
        <button
          onClick={() => openModal()}
                ...
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <img
                        src={u.avatar}
                        className="w-10 h-10 rounded-full border-2 border-slate-100 bg-white shadow-sm"
                        alt={u.name}
                      />
                      <div>
                        ...
                        <p className="text-[10px] text-jbGray font-bold">
                          {u.email}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="px-8 py-5">
                    <span
                      className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border ${u.role === "admin" ? "bg-jbBlue/10 text-jbBlue border-jbBlue/20" : "bg-slate-100 text-slate-600 border-slate-200"}`}
                    >
                      {u.role}
                    </span>
                  </td>
                  <td className="px-8 py-5">
                    <span
                      className={`flex items-center gap-1.5 text-[10px] font-black uppercase ${u.status === "active" ? "text-jbTurquoise" : "text-jbRed"}`}
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${u.status === "active" ? "bg-jbTurquoise" : "bg-jbRed"}`}
                      />
      ...
      {/* ── Modal editar / crear ── 🚀 Envuelta en Portal y usando inset-0 */}
      {isModalOpen &&
        createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in zoom-in duration-300">
            <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
              ...
              <form
                ...
                className="p-10 space-y-6 overflow-y-auto custom-scrollbar"
              >
                ...
                <div className="grid grid-cols-2 gap-6">
                    ...
                    {editingUser?.id === currentUser.id ? (
                      <div className="w-full bg-slate-100 border border-slate-200 rounded-2xl py-4 px-4 text-sm font-semibold text-slate-400 flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase">
                          {formData.role === "admin" ? "ADMIN" : "EMPLEADO"}
                        </span>
                  ...
                  <div className="space-y-2">
                    ...
                    {editingUser?.id === currentUser.id ? (
                      <div className="w-full bg-slate-100 border border-slate-200 rounded-2xl py-4 px-4 text-sm font-semibold text-slate-400 flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase text-jbTurquoise">
                          ACTIVO
                        </span>
                        <span className="text-[9px] text-slate-400 ml-auto">
                          No editable
                ...
                {error && (
                  <p className="text-jbRed text-[11px] font-bold bg-jbRed/10 py-3 px-4 rounded-xl border border-jbRed/20">
                    {error}
                  </p>
                )}
      ...
      {/* ✅ Modal de confirmación de eliminación 🚀 Envuelta en Portal y usando inset-0 */}
      {deleteModal.open &&
        deleteModal.user &&
        createPortal(
              ...
              <div className="p-8 text-center space-y-4">
                ...
                <div className="flex items-center gap-3 bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  <img
                    ...
                    className="w-12 h-12 rounded-full border-2 border-slate-200 bg-white"
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[10px] text-jbOrange font-black uppercase tracking-widest mt-0.5">
                      {deleteModal.user.area}
                    </p>
                  </div>
                </div>
                <p className="text-[11px] text-slate-400 font-semibold">
                  Esta acción eliminará al colaborador y todos sus registros de
                  asistencia. No se puede deshacer.
                </p>
              </div>
